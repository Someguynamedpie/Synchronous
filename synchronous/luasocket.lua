...
function wrapper:settimeout(n)
	self.timeout = n--Give up after this!
end
local ssl
function wrapper:dohandshake(params, sni)
...
function wrapper:receive(pattern, prefix, immediate) 
	...
	self.recvStart = self.timeout and gettime()
	while true do
		if self.timeout and (gettime() - self.recvStart) > self.timeout then
			return false, "timeout"
		end
		local suc, err = impl:addTask({self, 'recv'})
	...
	if #buffer == 0 and immediate then return false end
	return table.concat(buffer)
end
...
function impl:pollSockets(task, job)
	...
	elseif action:find'recv' then
		...
		if #r > 0 then
			self:wakeTask(task, true)
		elseif wrapper.recvStart and wrapper.timeout and (gettime() - wrapper.recvStart) > wrapper.timeout then
			self:wakeTask(task, false, "timeout")
		end
	end
